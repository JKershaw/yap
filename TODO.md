...
## Between v0.1 and v1.0 — live with it
...
13. **Reactive agent runtime.** `yap agent --config agent.json` as a new bin entry. Implements the listen-loop → LLM → say cycle. OpenRouter only to start. Test with a hand-rolled `planner.json`.

14. **CLI client.** `yap cli` as another bin entry. Terminal chat client against any server URL. Nice-to-have, not critical — skip if time-constrained.

15. **Web-based agent creator.** Form in the web UI: name, description, channels, system prompt, model. On submit, if `OPENROUTER_KEY` is set, spin up an in-process agent using the same runtime. If not, generate a downloadable `agent.json` the user can run locally with `yap agent`.
...
19. **Release v1.0.0.** Update the README. Tag the release. Write a short blog post if the mood strikes.
